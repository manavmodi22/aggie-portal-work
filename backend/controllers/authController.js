...
//     return next(
//       new ErrorResponse(
//         "E-mail already registered. If you think this is an error, please contact the system admin.",
//         400
//       )
//     );
//   }
...
exports.signup = async (req, res, next) => {
  ...
  try {
    const hashedPassword = await bcrypt.hash(password, 10);
    // Directly create the user with the default role and without verification code
    const user = await User.create({
      email,
      firstName,
      lastName,
      password: hashedPassword,
      role: -3, // Assuming role management is part of your schema
      status: "emailVerified" // Or any other status you use for active users
    });
    ...
    // Instead of sending a verification email, you might still want to send a welcome email
    // Or you can remove this part if no initial email is needed

    // Send a response indicating successful signup without the need for email verification
    res.status(201).json({
      success: true,
