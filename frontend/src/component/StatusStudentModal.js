...
const StudentStatusModal = ({ open, handleClose, studentId, currentStatus, handleChangeStatus }) => {
  const [selectedStatus, setSelectedStatus] = useState(currentStatus);

  const statuses = ['active', 'inactive', 'suspended', 'pending', 'emailVerified'];
    ...
    <Modal open={open} onClose={handleClose}>
      <Box sx={modalStyle}>
        ...
        <Box sx={{ mt: 2, display: 'flex', justifyContent: 'flex-end' }} className="button-container">
          ...
          <Button onClick={handleConfirm} className="button">Confirm Change</Button>
        </Box>
      </Box>
    </Modal>
  );
