...
const StudentDetailsModal = ({ open, onClose, student, refreshStudents }) => {
    ...
    if (open && student._id) {
      try {
        ...
        const response = await axios.get(
          `http://localhost:9000/api/students/${student._id}`
        );
  ...
  const handleConfirm = async () => {
    try {
      ...
      await axios.put(
        `http://localhost:9000/api/student/update/${studentDetails._id}`,
        formData,
        {
          headers: {
      ...
      refreshStudents();
      setSuccessMessage(
        `Student "${studentDetails.studentName}" updated successfully`
      );
  ...
  const handleDelete = async () => {
    if (
      ...
      try {
        await axios.delete(
          `http://localhost:9000/api/student/delete/${studentDetails._id}`
        );
        refreshStudents();
        setSuccessMessage(
          `Student "${studentDetails.studentName}" deleted successfully`
        );
        setShowSnackbar(true);
        setTimeout(() => {
            ...
            <div className="scrollable-table-container">
              <Table>
                <TableBody>
                  ...
                  {/* Resume section */}
                  <TableRow>
                    <TableCell component="th">Resume</TableCell>
                    <TableCell>
                      {studentDetails.resume && !isEditing && (
                        <Button
                          onClick={() =>
                            window.open(
                              `http://localhost:9000/api/students/resume/${studentDetails._id}`,
                              "_blank"
                            )
                          }
                        >
